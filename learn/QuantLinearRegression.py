...
class QuantLinearRegression(object):
  ...
  def train(self, dataset):
    num_features = dataset.getNumQuantitativeFeatures()
    params = np.zeros((num_features+1, 12))
    if self.debug:
      print "Running linear regression with %d quantitative features..." %(num_features)
